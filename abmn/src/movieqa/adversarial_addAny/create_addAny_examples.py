...
def run_creation(model_type, attack, model_folder, examples_folder, instances_to_attack):
    ...
    check_num = 0
    corr_probs = []
    if not tf.io.gfile.exists(examples_folder):
        tf.io.gfile.makedirs(examples_folder)
    else:
    ...
    add_sent = tf.compat.v1.placeholder(tf.int64, shape=[None])
    # sent_exp = tf.expand_dims(add_sent,0)
    m_p = tf.compat.v1.py_func(add_plot_sentence, [next_plots, add_sent], [tf.int64])[0]
    # m_p = next_plots
